...
VulkanPipeline::VulkanPipeline(VkDevice device, const PipelineCreateInfo& create_info) : device_(device) {
    // shaders
    auto vert_module = createShaderModule(device, create_info.vert_spv);
    auto frag_module = createShaderModule(device, create_info.frag_spv);
    ...
    vertexInputInfo.pVertexBindingDescriptions = create_info.vertex_binding_desc;
    vertexInputInfo.vertexAttributeDescriptionCount = create_info.attr_num;
    vertexInputInfo.pVertexAttributeDescriptions = create_info.attr_descs;
